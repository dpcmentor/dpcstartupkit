# Operations

This section covers day-to-day practice management and operational workflows for Direct Primary Care practices.

## Topics

- Office space selection and setup
- Staffing considerations
- Daily workflow and scheduling
- Inventory and supply management
- Patient onboarding processes
- Appointment scheduling optimization
- After-hours coverage
- Lab and imaging workflow
- Prescription management
- Quality improvement

## Guides

*Guides coming soon.*

## Checklists

- [ ] Determine space requirements and location criteria
- [ ] Secure office space (lease or purchase)
- [ ] Design office layout and patient flow
- [ ] Purchase equipment and supplies
- [ ] Develop staffing plan
- [ ] Hire and train staff (if applicable)
- [ ] Create patient onboarding workflow
- [ ] Establish scheduling protocols
- [ ] Set up supply ordering and inventory system
- [ ] Develop after-hours coverage plan
- [ ] Create emergency protocols
- [ ] Establish lab and imaging relationships

## Related Sections

- [Technology](../technology/) - Practice management software
- [Compliance and Legal](../compliance-legal/) - Operational compliance requirements

!!! tip "Talk to a mentor about this"
    Questions about day-to-day operations? A physician who has already done it will answer them, one to one and free.

    [Find a mentor](https://dpcmentor.com/mentors/browse/){ .md-button .dpckit-mentor-cta }
