# Compliance and Legal

This section covers regulatory requirements, legal considerations, and compliance obligations for Direct Primary Care practices.

## Topics

- State medical board requirements
- HIPAA compliance for small practices
- State-specific DPC legislation
- Regulatory considerations by state
- Documentation requirements
- Corporate practice of medicine rules
- Fee-splitting regulations
- Insurance and indemnity considerations
- Patient privacy and consent

## Guides

*Guides coming soon.*

## Checklists

- [ ] Review state medical board licensing requirements
- [ ] Research state-specific DPC laws and regulations
- [ ] Implement HIPAA compliance program
- [ ] Create Notice of Privacy Practices
- [ ] Develop patient consent forms
- [ ] Establish documentation policies
- [ ] Create Business Associate Agreements (BAAs)
- [ ] Implement data security measures
- [ ] Develop incident response procedures
- [ ] Create records retention policy

## Related Sections

- [Business Formation](../business-formation/) - Legal entity and insurance
- [Technology](../technology/) - HIPAA-compliant technology selection
- [Templates](../templates/) - Compliance document templates

!!! tip "Talk to a mentor about this"
    Questions about opting out, HIPAA or your state rules? A physician who has already done it will answer them, one to one and free.

    [Find a mentor](https://dpcmentor.com/mentors/browse/){ .md-button .dpckit-mentor-cta }
